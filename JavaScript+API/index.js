function Leer() {
    ...
    const api_url=`http://api.openweathermap.org/data/2.5/weather?q=${peli}&appid=${key}&units=metric` //unidades se cambian
    buscar3(api_url);
}
     
const buscar3=async(api_url)=>{

    const respuesta= await fetch(api_url); //cambiamos axion por fetch ya que la libreria no corria
